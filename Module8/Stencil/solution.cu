...
int main(int argc, char *argv[]) {
  ...
  gpuTKCheck(cudaMalloc((void **)&deviceInputData,
             width * height * depth * sizeof(float)));
  gpuTKCheck(cudaMalloc((void **)&deviceOutputData,
             width * height * depth * sizeof(float)));
  gpuTKTime_stop(GPU, "Doing GPU memory allocation");

  gpuTKTime_start(Copy, "Copying data to the GPU");
  gpuTKCheck(cudaMemcpy(deviceInputData, hostInputData,
             width * height * depth * sizeof(float),
  ...
  gpuTKCheck(cudaMemcpy(hostOutputData, deviceOutputData,
             width * height * depth * sizeof(float),
