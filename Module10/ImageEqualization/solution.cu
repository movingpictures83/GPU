...
int main(int argc, char **argv) {
  ...
  //@@ Allocate GPU memory
  gpuTKCheck(cudaMalloc((void **)&deviceImageData, imageHeight * imageWidth *
                                                    imageChannels *
                                                    sizeof(float)));
  gpuTKCheck(cudaMalloc((void **)&deviceCharImage,
                     imageHeight * imageWidth * imageChannels *
                         sizeof(unsigned char)));
  gpuTKCheck(cudaMalloc((void **)&deviceGrayImage,
                     imageHeight * imageWidth * sizeof(unsigned char)));
  gpuTKCheck(cudaMalloc((void **)&deviceHistogram,
                     HISTOGRAM_LENGTH * sizeof(unsigned int)));
  gpuTKCheck(
      cudaMalloc((void **)&deviceCDF, HISTOGRAM_LENGTH * sizeof(float)));
  gpuTKCheck(cudaMalloc((void **)&deviceCDFMin, 4 * sizeof(float)));

  //@@ Copy input memory, Memset
  gpuTKCheck(cudaMemset(deviceHistogram, 0,
                     HISTOGRAM_LENGTH * sizeof(unsigned int)));
  gpuTKCheck(cudaMemset(deviceCDF, 0, HISTOGRAM_LENGTH * sizeof(float)));
  gpuTKCheck(cudaMemset(deviceCharImage, 0, imageHeight * imageWidth *
                                             imageChannels *
                                             sizeof(unsigned char)));
  gpuTKCheck(cudaMemset(deviceGrayImage, 0,
                     imageHeight * imageWidth * sizeof(unsigned char)));
  ...
  gpuTKTime_start(GPU, "Cast image from float to unsigned char");
  castChar<<<DimGrid, DimBlock>>>(deviceImageData, deviceCharImage,
                                  imageWidth, imageHeight, imageChannels);
  cudaDeviceSynchronize();
  gpuTKTime_stop(GPU, "Cast image from float to unsigned char");

  //@@ Convert the image from RGB to GrayScale : Kernel
  gpuTKTime_start(GPU, "Convert the image from RGB to GrayScale");
  convertGrayScale<<<DimGrid, DimBlock>>>(deviceCharImage, deviceGrayImage,
                                          imageWidth, imageHeight,
                                          imageChannels);
  cudaDeviceSynchronize();
  gpuTKTime_stop(GPU, "Convert the image from RGB to GrayScale");

  //@@ Compute the histogram of grayImage : Kernel
  gpuTKTime_start(GPU, "Compute the histogram of grayImage");
  dim3 DimGrid2(1, 1, 1);
  histogram<<<DimGrid2, DimBlock>>>(deviceGrayImage, deviceHistogram,
                                    imageWidth, imageHeight);
  cudaDeviceSynchronize();
  gpuTKTime_stop(GPU, "Compute the histogram of grayImage");
  ...
  gpuTKTime_start(GPU, "Compute the minimum value of the CDF");
  cdfReduction<<<1, HISTOGRAM_LENGTH / 2>>>(deviceCDF, deviceCDFMin);
  cudaDeviceSynchronize();
  gpuTKTime_stop(GPU, "Compute the minimum value of the CDF");

  //@@ Apply the histogram equalization function: Kernel
  gpuTKTime_start(GPU, "Apply the histogram equalization function");
  applyHistogram<<<DimGrid, DimBlock>>>(
      deviceCharImage, deviceCDF, deviceCDFMin, deviceCharImage,
      imageWidth, imageHeight, imageChannels);
  cudaDeviceSynchronize();
  gpuTKTime_stop(GPU, "Apply the histogram equalization function");

  //@@Cast back to float - Kernel
  gpuTKTime_start(GPU, "Cast back to float");
  castFloat<<<DimGrid, DimBlock>>>(deviceCharImage, deviceImageData,
                                   imageWidth, imageHeight, imageChannels);
  cudaDeviceSynchronize();
  gpuTKTime_stop(GPU, "Cast back to float");
